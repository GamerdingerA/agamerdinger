...
setwd("~/Desktop/PhD/website/agamerdinger/content/teaching/virksomhedsstrategi/03-collecting-network-data")
...
gr %>% 
  ...
  geom_node_point(aes(filter=type==FALSE), color = "steelblue1", alpha = 1, size = 1) +
  geom_node_point(aes(filter=type==TRUE), color = "salmon1", alpha = 0.4, size = 2) +
...
comp1 %>% 
  ggraph("fr") +
  geom_edge_link0(color = "gray40") +
  geom_node_point(aes(filter=type==FALSE), color = "steelblue1", alpha = 1, size = 2) +
  geom_node_point(aes(filter=type==TRUE), color = "salmon1", alpha = 0.7, size = 4) +
  geom_node_text(aes(filter=type==TRUE, label = name), size = 2, repel = TRUE, force = 1) +
  labs(title = "Financial corporate interlocks in DK (biggest component)") +
  theme_graph()
